...
//        let dateSelected = dateFormatter.string(from: date1)
    ...
    func postDaysCount() -> Int? {
        guard let date = self.utcToLocal(format: .full1).changeToDateStandard(withFormat: .full1) else {
            let date = self.toDouble?.toDate.toString(format: .full1)
            return date?.postDaysCount()
        }
        let calendar = Calendar.current
        let difference = calendar.dateComponents([.minute, .hour, .day, .year], from: date, to: Date())
            ...
            if date.isToday {
                if difference.hour ?? 0 < 1 {
                    if difference.minute ?? 0 < 1 {
                        ...
                        return "\(difference.minute ?? 0) minutes ago"
                    }
                } else {
                    return "\(difference.hour ?? 0) hours ago"
                }
//                if UserDefaultsCustom.is24Hours {
//                    return date.toStringStandard(format: .HHmm)
//                } else {
//                    return date.toStringStandard(format: .time)
//                }
            } else {
                switch difference.day {
                case 0, 1 : return "1 day ago"
                case 2, 3, 4, 5, 6 : return "\(difference.day ?? 0) days ago"
                default:
//                    let time = UserDefaultsCustom.is24Hours ? "HH:mm" : "hh:mm a"// 'at' \(time)
                    return date.toStringStandard(format: "d MMM, yyyy")
                }
            }
        } else {
//            let time = UserDefaultsCustom.is24Hours ? "HH:mm" : "hh:mm a"'// 'at' \(time)
...
//        }
    ...
    func postDate() -> String? {
        guard let date = self.utcToLocal(format: .full1).changeToDateStandard(withFormat: .full1) else {
            let date = self.toDouble?.toDate.toString(format: .full1)
            return date?.postDate()
        }
        let calendar = Calendar.current
        let difference = calendar.dateComponents([.minute, .day, .year], from: date, to: Date())
        ...
        if difference.year == 0 {
            if date.isToday {
                ...
                case 2, 3, 4, 5, 6 : return "\(difference.day ?? 0) days ago"
                default:
//                    let time = UserDefaultsCustom.is24Hours ? "HH:mm" : "hh:mm a"
                    return date.toStringStandard(format: "dd MMM yyyy")
                }
            }
        } else {
//            let time = UserDefaultsCustom.is24Hours ? "HH:mm" : "hh:mm a"
    ...
    func postReviewDate() -> String? {
        guard let lmd = self.utcToLocal.changeToDateStandard(withFormat: .full1) else {
            return nil
        }
        let calendar = Calendar.current
        let diff = calendar.dateComponents([.minute, .hour, .day, .year], from: lmd, to: Date())
        if diff.year == 0 {
            if lmd.isToday {
                if diff.hour ?? 0 < 1 {
                    if diff.minute ?? 0 < 1 {
            ...
            } else if lmd.isYesterday {
                return "Yesterday"
            } else {
                return lmd.toStringStandard(format: "d MMM, yyyy")
            }
        } else {
            return lmd.toStringStandard(format: "d MMM, yyyy")
        }
