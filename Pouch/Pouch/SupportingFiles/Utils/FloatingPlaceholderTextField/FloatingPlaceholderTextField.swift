...
//  Pouch
//
//  Created by Raman choudhary on 22/02/23.
//
...
public class FloatingPlaceHolderTextFiled: UITextField {
    ...
    func Initialize(){
        ...
        self.addTarget(self, action: #selector(FloatingPlaceHolderTextFiled.textFieldDidChange), for: .editingChanged)
        self.EnableMaterialPlaceHolder(enableMaterialPlaceHolder: true)
        if isUnderLineAvailabe {
            let underLine = UIImageView()
            underLine.backgroundColor = self.placeHolderColor
    ...
    @IBInspectable var placeHolderColor: UIColor? = UIColor.red {
        didSet {
    ...
    override public var placeholder:String?  {
        ...
        willSet {
            let atts  = [NSAttributedString.Key.foregroundColor.rawValue: self.placeHolderColor!, NSAttributedString.Key.font: UIFont.labelFontSize] as! [NSAttributedString.Key : Any]
    ...
    func placeholderText(_ placeholder: NSString){
        let atts  = [NSAttributedString.Key.foregroundColor: self.placeHolderColor!, NSAttributedString.Key.font: UIFont.labelFontSize] as [NSAttributedString.Key : Any]
